/*
 * gerbil-aws-main.c — Custom entry point for gerbil-aws.
 *
...
#include <sys/mman.h>
#include "scheme.h"
#include "gerbil_aws_program.h"
#include "gerbil_aws_petite_boot.h"
#include "gerbil_aws_scheme_boot.h"
#include "gerbil_aws_app_boot.h"
...
int main(int argc, char *argv[]) {
    ...
    int fd = memfd_create("gerbil-aws-program", MFD_CLOEXEC);
    if (fd < 0) { perror("memfd_create"); return 1; }
    if (write(fd, gerbil_aws_program_data, gerbil_aws_program_size) != (ssize_t)gerbil_aws_program_size) {
    ...
    Sregister_boot_file_bytes("app", (void*)gerbil_aws_app_boot_data, gerbil_aws_app_boot_size);
