#!chezscheme
;; HTTP request compat module - wraps curl for HTTP operations
;; Replaces Gerbil's :std/net/request
(library (compat request)
  ...
  ;; A request result is a vector: #(status headers body)
  ...
  ;; Parse keyword args from rest list: 'headers: val 'data: val
  ...
  (define (run-curl method url headers data)
    (let* ([header-args
            (apply string-append
              (map (lambda (h)
                     (string-append " -H " (shell-quote (string-append (car h) ": " (cdr h)))))
                   headers))]
           [data-arg (if data
      ...
      (let-values ([(to-stdin from-stdout from-stderr pid)
        ...
        (let* ([output (get-string-all from-stdout)]
               ...
               [lines (string-split-last-line output)]
               [body (car lines)]
               [status-str (cdr lines)]
  ...
  (define (string-split-last-line s)
    ;; Returns (body . last-line) splitting on the last newline
