#!chezscheme
;; URI encoding compat module
;; Replaces Gerbil's :std/net/uri
(library (compat uri)
  (export form-url-encode uri-encode)
  (import (chezscheme))
             ...
             (let ([bv (string->utf8 (string c))])
               (do ([j 0 (+ j 1)])
                 ...
                 (let ([b (bytevector-u8-ref bv j)])
                   ...
                   (put-string out (number->string b 16)))))])))))

  ;; form-url-encode takes an alist of (key . value) pairs
  ;; and returns a URL-encoded form string
  (define (form-url-encode params)
    (let ([parts (map (lambda (p)
                        (string-append (uri-encode (if (symbol? (car p))
                                                      (symbol->string (car p))
                                                      (car p)))
                                       "="
  ...
  (define (string-join strs sep)
    (if (null? strs) ""
        (let loop ([rest (cdr strs)] [out (car strs)])
          (if (null? rest) out
              (loop (cdr rest)
                    (string-append out sep (car rest)))))))
)
